...
var InMemoryProjectsRepository = class {
  ...
  async delete(projectId) {
    const index2 = this.items.findIndex((project) => project.id === projectId);
  ...
  async addMember(projectId, userId, role) {
    const index2 = this.items.findIndex((project) => project.id === projectId);
    this.items[index2].members.push(userId);
  }
  ...
  async removeMember(projectId, userId) {
    ...
    if (!project || project.is_deleted)
      return;
    const index2 = project.members.indexOf(userId);
    if (index2 !== -1) {
      project.members.splice(index2, 1);
    }
  }
};
...
var InMemoryTestCasesRepository = class {
  ...
  async create(data) {
    const testCase = {
      id: data.id ?? (0, import_node_crypto2.randomUUID)(),
      title: data.title,
...
var GetTestCasesFromProjectUseCase = class {
  constructor(projectsRepository2, testCasesRepository2) {
    this.projectsRepository = projectsRepository2;
...
// src/use-cases/get-test-cases-from-project.spec.ts
...
describe("Get Test Cases From Project Use Case", () => {
  ...
  it("should be able to get the test cases from a specific project", async () => {
    ...
    globalExpect(testCases).toHaveLength(2);
    globalExpect(testCases).toEqual([
      globalExpect.objectContaining({ title: "Test Case 1" }),
      globalExpect.objectContaining({ title: "Test Case 2" })
    ]);
  });
